...
def msr(er, cov, riskfree_rate=0.03):
    """
...
def msr_weights(er, cov, riskfree_rate=0.1): 
    """
    Returns the weights for the portfolio the maximizes the Sharpe Ratio
    given a riskfree rate
    """
    msr_port = pd.DataFrame({'Weights':msr(riskfree_rate, er, cov)}, index=cov.columns)
    return msr_port[msr_port['Weights']>0.01]

def min_vol_weights(er, cov, target_return=0.1, cutoff=0.01):
    """
    Returns the weights for the portfolio the minimizes the volatility
    given a targeted return
    """
    w_minimize_vol = minimize_vol(target_return, er, cov)
    min_vol_port = pd.DataFrame({'Weights':w_minimize_vol}, index=cov.columns)
    return min_vol_port[min_vol_port['Weights']>cutoff]

def compare_portfolios(er, cov, riskfree_rate=0.10, target=0.10, cutoff=0.10):
    gmv_w = gmv(cov)
    msr_w = msr(riskfree_rate, er, cov)
    ew_w = 1 / cov.shape[0] 
    min_vol_w = min_vol_weights(er, cov, target_return=target, cutoff=cutoff)['Weights']
    ...
    portfolios['Min Vol'] = np.where(np.isnan(portfolios['Min Vol']), 0, portfolios['Min Vol'])
    return portfolios
 
def weight_returns(er, cov, df, port_type='EW'):
    compare_port = compare_portfolios(er, cov)
    weighted_rets = pd.DataFrame()
    for i in df.columns:
        weighted_rets[i] = df[i] * compare_port[port_type][i]
    weighted_rets['Total'] = weighted_rets.sum(axis=1)
...
def DCA(stock_close_data, monthly_allocation=1000):
    data = pd.DataFrame(index = stock_close_data.index)

    data['share price'] = stock_close_data
    data['pmt'] = np.repeat(monthly_allocation, stock_close_data.shape[0])
    data['shares month'] = stock_close_data
    data['shares total'] = 0
    data['principal'] = 0
    data['dca return'] = data['pmt']
    data['bm return'] = 0
    rets = stock_close_data.pct_change() 
    data['bm return'] = drawdown(rets, monthly_allocation)['Wealth']
    data['avg cost'] = 0
    
    for i in range(0, data.shape[0]):
        data['shares month'].iloc[i] = data['pmt'].iloc[i] / data['share price'].iloc[i]
        data['shares total'].iloc[i] = data['shares total'].iloc[i-1] + data['shares month'].iloc[i]
        data['principal'].iloc[i] = data['principal'].iloc[i-1] + data['pmt'].iloc[i]
        data['dca return'].iloc[i] = round(data['shares total'].iloc[i] * data['share price'].iloc[i], 2)
        data['avg cost'].iloc[i] = data['principal'].iloc[i] / data['shares total'].iloc[i] 
    return data

def DCA_selic(stock_close_data, monthly_allocation=1000):
    data = pd.DataFrame(index = stock_close_data.index)
    ...
    data['bm return'] = 0
    # rets = stock_close_data.pct_change() 
    # data['bm return'] = f.drawdown(rets, monthly_allocation)['Wealth']
    data['avg cost'] = 0
